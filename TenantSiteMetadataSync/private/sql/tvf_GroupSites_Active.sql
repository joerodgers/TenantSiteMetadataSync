IF OBJECT_ID('dbo.TVF_GroupSites_Active', 'IF') IS NOT NULL
   DROP FUNCTION [dbo].[TVF_GroupSites_Active]
GO

CREATE FUNCTION dbo.TVF_GroupSites_Active
(
)
...
WHERE
    (TimeDeleted IS NULL) AND (IsGroupConnected = 1)
