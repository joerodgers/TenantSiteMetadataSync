IF OBJECT_ID('dbo.TVF_Sites_Deleted', 'IF') IS NOT NULL
   DROP FUNCTION dbo.TVF_Sites_Deleted
GO

CREATE FUNCTION dbo.TVF_Sites_Deleted
(
)
...
SELECT        
    ...
    (SELECT MAX(VALUE) FROM 
        (
            SELECT DATEDIFF( DAY, GETDATE(), DATEADD( DAY, 92, TimeDeleted)) AS VALUE
            UNION
            SELECT 0 AS VALUE) AS T1
        ) AS 'DaysRemaining'
FROM    
