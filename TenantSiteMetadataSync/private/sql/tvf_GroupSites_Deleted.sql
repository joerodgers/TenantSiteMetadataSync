IF OBJECT_ID('dbo.TVF_GroupSites_Deleted', 'IF') IS NOT NULL
   DROP FUNCTION [dbo].[TVF_GroupSites_Deleted]
GO

CREATE FUNCTION dbo.TVF_GroupSites_Deleted
(
)
...
WHERE
    (TimeDeleted IS NOT NULL) AND (IsGroupConnected = 1)
